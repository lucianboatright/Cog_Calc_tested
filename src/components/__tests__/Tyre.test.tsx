...
import TyreIndex from "../Tyres/TyreIndex";

it.todo('should test that the components mounts')

it.todo('Should shown Tyre Size: without any selection')

it.todo('Should show 1,10,20,30 ans last when selected')
...
beforeEach(() => {
    render(<TyreIndex childToParentTyre={() => {}} childToParentTyreValue={() => {}} />)    
})  
...
// test('shoudd mount component', () => {
//     const tyreDisplay = screen.getByTestId(/tyre_selection_id'/i)
//     expect(tyreDisplay).toHaveBeenCalled(1)
// })

test('should mount component', () => {
    const tyreDisplay = screen.getByTestId(/WheelSize_Display/i)
    expect(tyreDisplay).toHaveTextContent('Tyre Size :')
})

test('should display 20.00 mm on choice #1', () => {
    const dropdown = screen.getByTestId('TyreSize_Dropdown_Drawer')
    fireEvent.click(dropdown)
    const dropDownOptions = getAllByTestId(dropdown, 'TyreSize_Dropdown_Styled')
    fireEvent.click(dropDownOptions[0])
    const valueDisplay = screen.getByTestId('WheelSize_Display')
    expect(valueDisplay).toHaveTextContent('Tyre Size : 20mm')
})

test('should display 27.00 mm on choice #4', () => {
    const dropdown = screen.getByTestId('TyreSize_Dropdown_Drawer')
    fireEvent.click(dropdown)
    const dropDownOptions = getAllByTestId(dropdown, 'TyreSize_Dropdown_Styled')
    fireEvent.click(dropDownOptions[4])
    const valueDisplay = screen.getByTestId('WheelSize_Display')
    expect(valueDisplay).toHaveTextContent('Tyre Size : 27mm')
})

test('should display 44.00 mm on choice #10', () => {
    const dropdown = screen.getByTestId('TyreSize_Dropdown_Drawer')
    fireEvent.click(dropdown)
    const dropDownOptions = getAllByTestId(dropdown, 'TyreSize_Dropdown_Styled')
    fireEvent.click(dropDownOptions[14])
    const valueDisplay = screen.getByTestId('WheelSize_Display')
    expect(valueDisplay).toHaveTextContent('Tyre Size : 44mm')
})

test('should display 1.90 Inch mm on choice #20', () => {
    const dropdown = screen.getByTestId('TyreSize_Dropdown_Drawer')
    fireEvent.click(dropdown)
    const dropDownOptions = getAllByTestId(dropdown, 'TyreSize_Dropdown_Styled')
    fireEvent.click(dropDownOptions[24])
    const valueDisplay = screen.getByTestId('WheelSize_Display')
    expect(valueDisplay).toHaveTextContent('Tyre Size : 1.90 Inch')
})

test('should display 2.50 Inch mm on choice #30', () => {
    const dropdown = screen.getByTestId('TyreSize_Dropdown_Drawer')
    fireEvent.click(dropdown)
    const dropDownOptions = getAllByTestId(dropdown, 'TyreSize_Dropdown_Styled')
    fireEvent.click(dropDownOptions[34])
    const valueDisplay = screen.getByTestId('WheelSize_Display')
    expect(valueDisplay).toHaveTextContent('Tyre Size : 2.50 Inch')
})

test('should display 5.00 mm on LAST choice', () => {
    const dropdown = screen.getByTestId('TyreSize_Dropdown_Drawer')
    fireEvent.click(dropdown)
    const dropDownOptions = getAllByTestId(dropdown, 'TyreSize_Dropdown_Styled')
    fireEvent.click(dropDownOptions[46])
    const valueDisplay = screen.getByTestId('WheelSize_Display')
    expect(valueDisplay).toHaveTextContent('Tyre Size : 5.00 Inch')
})

test('should change option when changed', () => {
    const dropdown = screen.getByTestId('TyreSize_Dropdown_Drawer')
    const valueDisplay = screen.getByTestId('WheelSize_Display')
    fireEvent.click(dropdown)
    const dropDownOptions = getAllByTestId(dropdown, 'TyreSize_Dropdown_Styled')
